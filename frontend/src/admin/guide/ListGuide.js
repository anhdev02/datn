...
const ListGuide = () => {
  ...
  useEffect(() => {
    document.title = 'Danh sách hướng dẫn';
    loadData();
  }, []);

  const loadData = () => {
    fetch("https://locknlock-dvxb.onrender.com/api/guide/list")
      .then((res) => res.json())
      .then((data) => {
        setGuide(data);
      })
  ...
  const addTrash = async (event) => {
    event.preventDefault();
    const result = await confirm("Bạn có chắc muốn xóa hướng dẫn?", event);
    if (result) {
      var id = event.target.id;
      var url = "https://locknlock-dvxb.onrender.com/api/guide/" + id;
      ...
      fetch(url, requestOptions)
        ...
        .then((result) => {
          console.log(result);
          toast.success("Đã xóa tạm thời hướng dẫn!", { position: "top-right" });
          loadData();
        })
                        ...
                        <div className="dropdown">
                          ...
                          <div className="dropdown-menu">
                            <Link
                              ...
                              to={"/dashboard/guide/edit/" + item.id}
                            >
