...
const ListSlide = () => {
  ...
  useEffect(() => {
    document.title = 'Danh sách slide';
    loadData();
  }, []);

  const loadData = () => {
    fetch('https://locknlock-dvxb.onrender.com/api/slide/list')
    .then((res) => res.json())
    .then((data) => {
      setSlide(data);
    })
  ...
  const addTrash = async (event) => {
    event.preventDefault();
    const result = await confirm("Bạn có chắc muốn xóa slide?", event);
    if (result) {
                        ...
                        <div className="dropdown">
                          ...
                          <div className="dropdown-menu">
                            <Link
                              ...
                              to={"/dashboard/slide/edit/" + item.id}
                            >
